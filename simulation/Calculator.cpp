...
double Calculator::calcDistance(Star& s1, Star& s2) {
    const std::vector<double>& pos1 = s1.position;
    const std::vector<double>& pos2 = s2.position;
    ...
    for (int val = 0; val < pos1.size(); ++val) {
        const double displacement = pos2[val] - pos1[val];
...
double Calculator::calcDistance(Node* s1, Node* s2) {
